...
router.post("/:adminId/create/camp", isLoggedIn,  async (req, res) => {
...
router.get("/:adminId/camps", isLoggedIn,  async (req, res) => {
  let { adminId } = req.params;
  let camps = await Camp.find({ adminId }).populate("AssignDoctor");
  res.json(camps);
});

router.post("/:adminId/create/doctor", isLoggedIn,  async (req, res) => {
  let { adminId } = req.params;
  console.log(req.body);
...
router.get("/:adminId/card-details", isLoggedIn, async (req, res) => {
  let { adminId } = req.params;
